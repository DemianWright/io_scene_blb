...
def __get_sequence_string(sequence, new_line=True, decimal_digits=const.MAX_FP_DECIMALS_TO_WRITE):
    ...
    for index, value in enumerate(sequence):
        ...
        if value == 0:
            ...
            if decimal_digits is None:
                parts.append("{}".format(value).rstrip("0").rstrip("."))
            else:
...
def write_file(properties, filepath, blb_data):
    ...
    # Brick Size
    # ----------
    lines.append(__get_sequence_string(blb_data.brick_size))
    ...
    if len(blb_data.collision) < 1:
        ...
        for (center, dimensions) in blb_data.collision:
            ...
            lines.append(__get_sequence_string(dimensions))
